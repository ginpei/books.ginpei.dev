...
最後の配列要素の次にカンマを置いてもよい。（無視される。）

最後以外でも配列要素なしにカンマ `,` を置くことができる。その場合、配列の要素数は増えるにも関わらず配列要素は存在せず "empty item(s)" となる。これは `undefined` が設定されている状態とは異なり、`map()` 等の繰り返しでも呼ばれない。配列要素を得ようとする場合は（該当の配列要素が存在しない結果として） `undefined` になる。
...
該当の配列要素の存在は `in` 演算子で確認できる。

```js
const a = [, undefined];
const b = '0' in a; // => false
const c = '1' in a; // => true
```

なお `fill()` は他の多くの配列操作関数とは異なり empty item の位置にも値を設定する。

```js
const a = [,, 33]; // => [ <2 empty items>, 33 ]
const b = a.map(() => 0); // => [ <2 empty items>, 0 ]
a.fill(0);
console.log(a); // => [ 0, 0, 0 ]
```
